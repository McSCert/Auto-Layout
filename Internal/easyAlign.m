function layout = easyAlign(layout)
    ...
    for j = 1:length(layout) % for each column
        for i = length(layout{j}):-1:1 % for each non empty row in column (reverse order)
            block1 = layout{j}{i}; % block to align
            [layout, aligned] = align(layout, aligned, block1, j);
        end
    end
        ...
        if length(portCon1(end).DstBlock) ~= 1 %>=2, therefore branching line
            ...
            if ports1(1) == 1 && ~strcmp(aligned(block1), 'triedleft')
                ...
                if shamt < 0
                    ...
                    tmpPos = get_param(layout{col}{row-1}, 'Position');
                    if row == 1 || tmpPos(4) < curPos(2) + shamt
                ...
                elseif shamt > 0
                    curPos = get_param(layout{col}{row}, 'Position');
                    tmpPos = get_param(layout{col}{row+1}, 'Position');
                    if row == length(layout{col}) || tmpPos(2) > curPos(4) + shamt
                        set_param(layout{col}{row}, 'Position', curPos + [0, shamt, 0, shamt]);
                        aligned(block1) = 'aligned';
                    end
                else
