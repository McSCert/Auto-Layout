function layout = vertAlign(layout)
    ...
    % connections by repositioning blocks vertically. Currently only
    % attempts to align blocks which connect to a single block through an
    % inport/outport.
    %
    %   Inputs:
    %       layout  Cell array of columns. Columns are represented by a
    %               cell array of blocks (fullnames or handles).
    %
    %   Outputs:
    %       layout  Updated with columns ordered by top position (may not
    %               be likely to change).
    ...
    while(loop)
        ...
        for i = length(blocksToTryAlign):-1:1 % Reverse order to not mess up indices since items will be removed
            ...
            block1 = layout{col}{row}; % block to align
            ...
            curBounds = getPositionWithName(getfullname(layout{col}{row}));
            if shamt < 0
                if row == 1
                    ...
                    blockBounds = getPositionWithName(getfullname(layout{col}{row-1}));
            ...
            if doAlign
                %Align block
                curPos = get_param(layout{col}{row}, 'Position');
                set_param(layout{col}{row}, 'Position', curPos + [0, shamt, 0, shamt]);
