...
%       It chooses which half the system to place the block in based on the half it started in
    ...
    fixBlockSize(blocksMatrix, colLengths);
    ...
    colDims = getColumnDimensions(blocksMatrix, colLengths);
    vSegs = getVSegs(systemLines);

    % Adjust net horizontal space between columns in blocksMatrix
    adjustHorzSpacing(blocksMatrix, colLengths, vSegs, colDims);
    redraw_lines(address,'autorouting','on');
...
% Expands space between blocks within columns of blocks if less than a minimum
    ...
    for j = 1:size(blocksMatrix, 2) % For each column
        ...
        for i = 1:colLengths(j) - 1 % For each non-empty row in column except last
            pos2 = get_param(char(blocksMatrix{i+1, j}), 'Position');
            freeSpace = pos2(2) - pos1(4);

            string = get_param(blocksMatrix{i, j}, 'Name');
            minSpace = getTextHeight(string, char(blocksMatrix{i, j}));

            if freeSpace < minSpace
                adjustment = minSpace - freeSpace;
                vertMoveColumn(blocksMatrix, colLengths, i, j, adjustment);
            end
...
% by the number of vertical segments in that space.
    ...
    for i = 2:length(colDims) % For each column after first
        ...
        if freeSpace < minSpace
            adjustment = minSpace - freeSpace;
            horzMoveBlocks(blocksMatrix, colLengths, i-1, adjustment);
        end
    end
end
...
%   colDims{#} = [largestLeftPosition, largestRightPosition]
    ...
    for j = 1:size(blocksMatrix, 2) % For columns in blocksMatrix
        ...
        largestRight = 0;
        for i = 1:colLengths(j) % For each non-empty in column
            if lowestLeft > getBlockSidePositions(blocksMatrix{i,j}, 1) % 1->left side
                lowestLeft = getBlockSidePositions(blocksMatrix{i,j}, 1);
            end
            if largestRight < getBlockSidePositions(blocksMatrix{i,j}, 3) % 3->right side
                largestRight = getBlockSidePositions(blocksMatrix{i,j}, 3);
            end
        end
