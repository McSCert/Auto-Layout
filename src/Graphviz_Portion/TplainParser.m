classdef TplainParser < handle
    ...
    properties
        Blocks  % Simulink model name (or top-level system name).
        Filename        % Name of the Graphviz output txt file.
        Map             % (See dotfile_creator).
    end
    
    methods
        function object = TplainParser(blocks, filename, replacementMap)
            ...
            %
            %   Inputs:
            %       blocks          Vector of block handles in which each
            %                       block is at the top level of the same
            %                       system.
            %       filename        Name of the Graphviz output txt file.
...
%             end
        ...
        function find_the_right_spot(object, mapObj, graphinfo)
            ...
            for z = 1:blocklength
                ...
                setPositionAL(object.Blocks(z), pos);
            end
        end
    end
end